...
impl<'a> LexString<'a> {
    ...
    pub fn remain_string(&mut self) -> String {
        let mut remain = self.chars.as_str().to_string();
        for idx in 0..self.ahead_count {
            let chr = self.cache_list[self.cache_list.len() - (idx + 1)];
            remain.insert(0, chr);
        }
        remain
    }
...
// ∀ 1 2 3 4 5 6 7
    ...
    pub fn look_string(&mut self,str:&str) -> bool {
        let mut index = 1;
